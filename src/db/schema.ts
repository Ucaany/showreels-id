...
export const users = pgTable(
  "users",
  {
    ...
    showPortfolioButton: boolean("show_portfolio_button").notNull().default(true),
...
export const siteSettings = pgTable("site_settings", {
  ...
  defaultPaymentMethod: text("default_payment_method").notNull().default("bayar_gg"),
  updatedAt: timestamp("updated_at", { mode: "date" }).notNull().defaultNow(),
});
...
export const creatorSettings = pgTable(
  "creator_settings",
  {
    ...
    paymentMethod: text("payment_method").notNull().default("bayar_gg"),
...
export const billingTransactions = pgTable(
  "billing_transactions",
  {
    ...
    status: text("status")
      ...
      .notNull()
      .default("pending"),
    provider: text("provider").notNull().default("bayar_gg"),
    providerReference: text("provider_reference").notNull().default(""),
    snapToken: text("snap_token").notNull().default(""), // deprecated — legacy field, unused
...
// ??? Website Audit Tables ???????????????????????????????????????????????????

export const auditScans = pgTable("audit_scans", { id: uuid("id").primaryKey().defaultRandom(), targetUrl: text("target_url").notNull(), scope: text("scope").notNull().default("full"), status: text("status").$type<"queued" | "running" | "completed" | "failed" | "cancelled">().notNull().default("queued"), triggerType: text("trigger_type").$type<"manual" | "cron" | "deploy">().notNull().default("manual"), startedAt: timestamp("started_at", { mode: "date" }), finishedAt: timestamp("finished_at", { mode: "date" }), durationMs: integer("duration_ms").notNull().default(0), healthScore: integer("health_score").notNull().default(100), lowCount: integer("low_count").notNull().default(0), mediumCount: integer("medium_count").notNull().default(0), highCount: integer("high_count").notNull().default(0), criticalCount: integer("critical_count").notNull().default(0), summaryJson: jsonb("summary_json").$type<Record<string, unknown>>().notNull().default(sql`'{}'::jsonb`), errorMessage: text("error_message").notNull().default(""), createdBy: uuid("created_by").references(() => users.id, { onDelete: "set null" }), createdAt: timestamp("created_at", { mode: "date" }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { mode: "date" }).notNull().defaultNow() }, (table) => ({ statusIdx: index("audit_scans_status_idx").on(table.status), createdAtIdx: index("audit_scans_created_at_idx").on(table.createdAt), triggerIdx: index("audit_scans_trigger_type_idx").on(table.triggerType) }));

export const auditFindings = pgTable("audit_findings", { id: uuid("id").primaryKey().defaultRandom(), scanId: uuid("scan_id").notNull().references(() => auditScans.id, { onDelete: "cascade" }), category: text("category").notNull(), severity: text("severity").$type<"low" | "medium" | "high" | "critical">().notNull(), status: text("status").$type<"open" | "resolved" | "ignored">().notNull().default("open"), code: text("code").notNull(), title: text("title").notNull(), description: text("description").notNull().default(""), routeOrEndpoint: text("route_or_endpoint").notNull().default(""), evidenceJson: jsonb("evidence_json").$type<Record<string, unknown>>().notNull().default(sql`'{}'::jsonb`), recommendation: text("recommendation").notNull().default(""), fingerprint: text("fingerprint").notNull(), createdAt: timestamp("created_at", { mode: "date" }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { mode: "date" }).notNull().defaultNow() }, (table) => ({ scanIdx: index("audit_findings_scan_id_idx").on(table.scanId), severityIdx: index("audit_findings_severity_idx").on(table.severity), categoryIdx: index("audit_findings_category_idx").on(table.category), statusIdx: index("audit_findings_status_idx").on(table.status), fingerprintIdx: index("audit_findings_fingerprint_idx").on(table.fingerprint) }));

export const auditRouteChecks = pgTable("audit_route_checks", { id: uuid("id").primaryKey().defaultRandom(), scanId: uuid("scan_id").notNull().references(() => auditScans.id, { onDelete: "cascade" }), route: text("route").notNull(), statusCode: integer("status_code").notNull().default(0), loadTimeMs: integer("load_time_ms").notNull().default(0), payloadBytes: integer("payload_bytes").notNull().default(0), hasTitle: boolean("has_title").notNull().default(false), hasMetaDescription: boolean("has_meta_description").notNull().default(false), errorMessage: text("error_message").notNull().default(""), createdAt: timestamp("created_at", { mode: "date" }).notNull().defaultNow() }, (table) => ({ scanIdx: index("audit_route_checks_scan_id_idx").on(table.scanId), routeIdx: index("audit_route_checks_route_idx").on(table.route) }));

export const auditApiChecks = pgTable("audit_api_checks", { id: uuid("id").primaryKey().defaultRandom(), scanId: uuid("scan_id").notNull().references(() => auditScans.id, { onDelete: "cascade" }), endpoint: text("endpoint").notNull(), method: text("method").notNull().default("GET"), statusCode: integer("status_code").notNull().default(0), latencyMs: integer("latency_ms").notNull().default(0), ok: boolean("ok").notNull().default(false), errorMessage: text("error_message").notNull().default(""), createdAt: timestamp("created_at", { mode: "date" }).notNull().defaultNow() }, (table) => ({ scanIdx: index("audit_api_checks_scan_id_idx").on(table.scanId), endpointIdx: index("audit_api_checks_endpoint_idx").on(table.endpoint) }));

export const systemHealthHistory = pgTable("system_health_history", { id: uuid("id").primaryKey().defaultRandom(), scanId: uuid("scan_id").references(() => auditScans.id, { onDelete: "set null" }), healthScore: integer("health_score").notNull(), criticalCount: integer("critical_count").notNull().default(0), highCount: integer("high_count").notNull().default(0), mediumCount: integer("medium_count").notNull().default(0), lowCount: integer("low_count").notNull().default(0), createdAt: timestamp("created_at", { mode: "date" }).notNull().defaultNow() }, (table) => ({ createdAtIdx: index("system_health_history_created_at_idx").on(table.createdAt) }));
...
export type DbAuditScan = typeof auditScans.$inferSelect;
export type NewDbAuditScan = typeof auditScans.$inferInsert;
export type DbAuditFinding = typeof auditFindings.$inferSelect;
export type NewDbAuditFinding = typeof auditFindings.$inferInsert;
