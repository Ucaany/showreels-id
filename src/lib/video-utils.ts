...
function extractFacebookId(url: string): string | null {
  ...
  if (segments[0] === "video.php" || segments[0] === "watch") {
...
function extractTiktokId(url: string): string | null {
  ...
  // vm.tiktok.com short links need redirect resolution to obtain numeric video id.
  // Without server-side resolution we cannot generate a reliable embed URL.
  ...
  // Format: /embed/v2/1234567890123456789
...
export function getEmbedReadyVideoUrl(url: string): EmbedReadyVideoUrl | null {
  ...
  if (tiktokId) {
    ...
    const canonicalUrl =
      host === "tiktok.com" && normalized.includes("/@") && normalized.includes("/video/")
        ? normalized
