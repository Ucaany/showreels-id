...
import { getPlanFeatureChecklist } from "@/lib/plan-feature-matrix";
...
export function PricingSubscriptionPage({
  ...
  const paymentReady = paymentConfig.configured && billingEnabled;
  ...
  const createPaidTransaction = useCallback(async (planName: "creator" | "business") => {
    ...
    if (!response.ok || payload?.mode !== "paid" || !payload.payment) {
      // Pesan ramah user — JANGAN pernah menampilkan detail credential atau
      // pesan mentah dari payment gateway (mis. IP whitelist) ke end-user.
      // Detail teknis tetap dicatat di server logs untuk admin.
      ...
      if (code === "bayar_gg_auth_error") {
        message =
          locale === "en"
            ? "Payment configuration issue. Our team has been notified. Please try again later."
            : "Konfigurasi pembayaran sedang bermasalah. Tim kami sudah dinotifikasi. Silakan coba lagi nanti.";
      } else if (code === "bayar_gg_network_error") {
        message =
          locale === "en"
            ? "Cannot reach payment server. Please check your connection and try again."
            : "Tidak dapat terhubung ke server pembayaran. Periksa koneksi lalu coba lagi.";
      } else if (code === "bayar_gg_not_configured") {
        message =
          locale === "en"
            ? "Bayar.gg is not configured yet. Please contact admin."
            : "Bayar.gg belum dikonfigurasi. Silakan hubungi admin.";
      } else if (code === "bayar_gg_validation_error") {
        message =
          locale === "en"
            ? "Payment request is invalid. Please recreate the transaction."
            : "Permintaan pembayaran tidak valid. Silakan buat ulang transaksi.";
      }
    ...
    const paymentUrl = payload.payment_url || payload.payment.checkoutUrl;
    if (!paymentUrl) {
      const message =
        locale === "en"
        ...
        <section className="rounded-[2rem] border border-[#dfd6d0] bg-white p-4 shadow-[0_24px_48px_-34px_rgba(26,38,63,0.34)] sm:p-7 lg:p-9">
          ...
          {!isLoggedIn ? (
            <Card className="mt-6 border-[#cfe0ff] bg-[#f4f8ff] p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <Link href={`/auth/login?next=${encodeURIComponent(`/payment?plan=${selectedPlan}`)}`}>
                    ...
                    <div
                      className={cn(
                        ...
                        plan.featured
                          ...
                          : "border-[#fed7aa] bg-[#fff7ed] text-[#7c2d12]"
                      )}
                    >
          ...
          {flowStep === "confirmation" ? (
            <Card className="mt-5 border-[#d9cec7] bg-white p-4 sm:p-5">
              ...
              {selectedPlan === "business" ? (
                <div className="mt-3 rounded-xl border border-[#fed7aa] bg-[#fff7ed] p-3 text-sm text-[#1f3f84]">
