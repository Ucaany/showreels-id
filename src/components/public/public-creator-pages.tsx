...
const darkButtonClass =
  "btn-pill bg-[#111111] !text-white shadow-[0_14px_30px_rgba(17,17,17,0.16)] hover:bg-[#1E1E1E] hover-glow focus:outline-none focus:ring-2 focus:ring-[#111111]/25 disabled:bg-[#3A3A3A] disabled:text-[#DADADA] [&_svg]:text-white";
const monoButtonClass =
  "btn-pill w-full border border-white/30 bg-white/50 backdrop-blur-sm px-4 text-sm font-semibold text-[#111111] transition hover:-translate-y-0.5 hover:border-[#111111] hover:bg-white/70 hover-glow focus:outline-none focus:ring-2 focus:ring-[#111111]/20";
  ...
  return (
    <div className={pageShellClass}>
      ...
      <main className="mx-auto flex min-h-screen w-full max-w-[460px] flex-col justify-center px-4 py-6 min-[481px]:max-w-[560px] sm:px-5 sm:py-10 lg:max-w-[640px]">
        <Card className={`${cardClassStrong} overflow-hidden rounded-[2.5rem] p-4 sm:p-5 lg:p-6`}>
          ...
          <div className="-mt-12 flex flex-col items-center text-center sm:-mt-14">
            <div className="relative z-10 rounded-full border-4 border-white/60 bg-white/40 backdrop-blur-sm shadow-[0_14px_34px_rgba(17,17,17,0.12)]">
            ...
            <h1 className="mt-5 max-w-full text-[length:var(--type-public-hero)] font-bold leading-tight tracking-[-0.04em] text-[#111111]">
            ...
            <div className="mt-7 w-full space-y-3 sm:space-y-4">
              {pinnedVideos.map((video) => {
                ...
                return (
                  <Link key={video.id} href={getVideoDetailHref(video.publicSlug)} className="group block">
                    <article className="flex min-h-[104px] items-center gap-3 rounded-[2rem] border border-white/30 bg-white/40 backdrop-blur-sm p-2.5 text-left transition hover:border-[#111111]/40 hover:bg-white/60 hover-glow max-[359px]:block">
              ...
              {profile.user.customLinks.length === 0 && pinnedVideos.length === 0 ? (
                <p className="rounded-[2rem] border border-dashed border-white/40 bg-white/30 backdrop-blur-sm px-4 py-4 text-center text-sm text-[#525252]">Creator belum menambahkan link.</p>
              ) : null}
    ...
    <div className={pageShellClass}>
      <main className="mx-auto w-full max-w-7xl px-4 py-5 sm:px-6 lg:py-8">
        ...
        <div className="mb-5 flex items-center justify-between gap-3">
          <Link href={bioHref} className="inline-flex min-h-11 items-center gap-2 btn-pill border border-white/30 bg-white/50 backdrop-blur-sm px-4 text-sm font-semibold text-[#111111] shadow-sm transition hover:border-[#111111] hover:shadow-md hover-glow"><ArrowLeft className="h-4 w-4" />Kembali</Link>
        ...
        <Card className="mb-4 overflow-hidden rounded-[2.5rem] border border-white/40 bg-white/50 p-4 shadow-[0_18px_50px_rgba(17,17,17,0.06)] backdrop-blur-sm sm:p-5 lg:p-6">
          {/* Mobile: centered avatar + info */}
          <div className="flex flex-col items-center text-center lg:hidden">
            <div className="rounded-full border-4 border-white/60 bg-white/40 backdrop-blur-sm shadow-[0_12px_26px_rgba(17,17,17,0.12)]">
              <CreatorAvatar profile={profile} />
            </div>
            <p className="mt-3 text-xs font-semibold uppercase tracking-[0.24em] text-[#8A8A8A]">Portfolio Creator</p>
            <h1 className="mt-2 text-[length:var(--type-public-section)] font-bold tracking-[-0.04em] text-[#111111]">
              {profile.user.name || "Creator"}
              {isProfileVerified(profile) && <VerifiedBadge className="ml-2 align-middle" />}
            </h1>
          ...
          <div className="hidden lg:block">
            <div className="relative overflow-hidden rounded-2xl border border-white/25 bg-white/40 p-7 backdrop-blur-sm">
              <div className="flex items-center gap-5">
                <div className="shrink-0 rounded-full border-4 border-white/60 bg-white/40 backdrop-blur-sm shadow-[0_12px_26px_rgba(17,17,17,0.12)]"><CreatorAvatar profile={profile} /></div>
                <div className="min-w-0">
                  <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#8A8A8A]">Portfolio Creator</p>
                  <h1 className="mt-2 text-[length:var(--type-public-section)] font-bold tracking-[-0.04em] text-[#111111] xl:text-4xl">
                    {profile.user.name || "Creator"}
                    {isProfileVerified(profile) && <VerifiedBadge className="ml-2 align-middle" />}
                  </h1>
                  <p className="mt-1 text-sm font-semibold text-[#525252]">@{profile.user.username} {profile.user.role ? `• ${profile.user.role}` : ""}</p>
        ...
        <div className="wave-separator my-3" aria-hidden="true" />

        {/* Filter / Toggle Bar */}
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#8A8A8A]">Video Portfolio</p>
            <h2 className="mt-1 text-[length:var(--type-public-section)] font-bold tracking-[-0.03em]">Karya public terbaru</h2>
          </div>
          <div className="flex max-w-full items-center gap-1.5 overflow-x-auto rounded-full border border-white/30 bg-white/50 p-1 shadow-sm backdrop-blur-sm">
            <span className="inline-flex min-h-10 shrink-0 items-center gap-2 rounded-full border border-white/30 bg-white/60 px-3.5 text-sm font-semibold text-[#525252]"><Search className="h-4 w-4" />Semua</span>
            <Link href={`${getCreatorPortfolioHref(username)}?view=grid`} className={`inline-flex min-h-10 shrink-0 items-center gap-2 rounded-full px-3.5 text-sm font-semibold transition ${!isList ? "bg-[#111111] text-white shadow-[0_4px_16px_rgba(17,17,17,0.25)]" : "border border-white/40 bg-white/70 text-[#525252] hover:border-[#111111]"}`}><Grid2X2 className="h-4 w-4" />Grid</Link>
        ...
        {/* Video Grid */}
        {profile.videos.length === 0 ? (
          <Card className="rounded-3xl border border-white/40 bg-white/60 p-8 text-center shadow-[0_18px_50px_rgba(17,17,17,0.06)] backdrop-blur-sm">
            ...
            <Link href={bioHref} className={`mt-5 inline-flex min-h-11 items-center justify-center rounded-full border border-white/30 bg-white/60 px-5 text-sm font-bold hover-glow ${darkButtonClass}`}>Kembali ke Bio</Link>
          </Card>
        ) : (
...
function PortfolioVideoCard({ video, list }: { video: ProfileVideo; list?: boolean }) {
  ...
  return (
    <Link href={getVideoDetailHref(video.publicSlug)} className="group block min-w-0">
      <article className={`glass-panel h-full rounded-[2rem] p-2.5 transition hover:-translate-y-1.5 hover:border-white/50 hover-glow sm:p-3 ${list ? "md:flex md:gap-4" : ""}`}>
        <div className={`relative overflow-hidden rounded-[1.25rem] bg-[#EFEDEA] ${list ? "md:w-72 md:shrink-0" : ""}`}>
          ...
          {/* Floating badges over thumbnail */}
...
export function VideoDetailPublicPage({ video }: { video: PublicVideo }) {
  ...
  const sourceLabel = getSourceLabel(video.source as never);
  const thumbBg = video.thumbnailUrl || getAutoThumbnailFromVideoUrl(video.sourceUrl) || "";

  return (
    <div className={pageShellDarkClass}>
      ...
      <main className="relative z-10 mx-auto w-full max-w-7xl px-4 py-5 sm:px-6 lg:py-8">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          <Link href={portfolioHref} className="btn-pill btn-pill-sm border border-white/20 bg-white/10 backdrop-blur-md px-4 text-sm font-semibold text-[#f5f5f4] hover:bg-white/20"><ArrowLeft className="h-4 w-4" />Kembali ke Portfolio</Link>
          <Link href={video.sourceUrl} target="_blank" rel="noopener noreferrer" className="btn-pill btn-pill-sm border border-white/20 bg-white/15 backdrop-blur-md px-4 text-sm font-bold text-white hover:bg-white/25 max-[420px]:w-full max-[420px]:justify-center">Buka Source <ArrowUpRight className="h-4 w-4" /></Link>
        </div>

        {/* Mobile: Unified panel (description + info merged) */}
        <div className="space-y-5 lg:hidden">
          ...
          <div className="glass-panel-dark rounded-[2rem] p-5 sm:p-7">
            ...
            <p className="mt-5 whitespace-pre-line text-base leading-8 text-[#a1a1aa]">{video.description || "Deskripsi project belum ditambahkan."}</p>
          ...
          {video.author ? (
            <div className="glass-panel-dark rounded-[2rem] p-5">
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#a1a1aa]">Creator</p>
              <div className="mt-4 flex items-start gap-3">
                ...
                <div className="min-w-0">
                  <h2 className="font-bold text-[#f5f5f4]">{video.author.name || "Creator"}</h2>
                  <p className="text-sm font-semibold text-[#a1a1aa]">@{video.author.username || "creator"}</p>
                  {video.author.role ? <p className="mt-1 text-sm text-[#a1a1aa]">{video.author.role}</p> : null}
                </div>
              </div>
              <div className="mt-4 grid gap-2">
                <Link href={creatorHref} className="btn-pill min-h-12 justify-center bg-white/15 border border-white/20 backdrop-blur-md text-sm font-bold text-white hover:bg-white/25">Lihat Bio</Link>
                <Link href={portfolioHref} className="btn-pill min-h-12 justify-center border border-white/20 bg-white/5 text-sm font-bold text-[#f5f5f4] hover:bg-white/15">Lihat Semua Portofolio</Link>
              </div>
            </div>
          ) : null}

          <div className="glass-panel-dark rounded-[2rem] p-5">
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#a1a1aa]">Share</p>
            <h2 className="mt-2 text-2xl font-bold tracking-[-0.03em] text-[#f5f5f4]">Bagikan Project</h2>
        ...
        <div className="hidden lg:grid lg:grid-cols-[minmax(0,1fr)_360px] lg:gap-5">
          <div className="space-y-5">
            ...
            <div className="glass-panel-dark rounded-[2rem] p-5 sm:p-7">
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#a1a1aa]">Project Description</p>
              <h1 className="mt-3 text-[2rem] font-bold leading-tight tracking-[-0.04em] text-[#f5f5f4] sm:text-5xl">{video.title}</h1>
              <p className="mt-5 whitespace-pre-line text-base leading-8 text-[#a1a1aa]">{video.description || "Deskripsi project belum ditambahkan."}</p>
            </div>
          </div>
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex min-h-11 flex-wrap items-center justify-between gap-2 rounded-[1.25rem] border border-white/10 bg-white/5 px-3 py-2 text-sm">
      <span className="font-medium text-[#71717a]">{label}</span>
      <span className="min-w-0 break-words text-right font-bold text-[#f5f5f4]">{value}</span>
    </div>
  );
}
