  ...
  return (
    <div className={pageShellClass}>
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-5 sm:px-6 lg:py-8">
        ...
        <Card className={`${cardClass} mb-6 overflow-hidden rounded-[2rem] p-4 sm:p-5 lg:p-7`}>
          <div className="grid gap-5 lg:min-h-[240px] lg:grid-cols-[1.2fr_0.8fr] lg:items-stretch">
            <div className="relative overflow-hidden rounded-[1.75rem] border border-[#EFEDEA] bg-[#FAFAF9] p-5 sm:p-6 lg:p-7">
              <CreatorCover profile={profile} soft className="absolute inset-0 h-full rounded-[1.75rem] border-0 lg:hidden" />
            ...
            <CreatorCover profile={profile} soft className="hidden h-full min-h-[220px] rounded-[1.75rem] lg:block" />
          </div>
        </Card>
        ...
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex max-w-full items-center gap-2 overflow-x-auto rounded-full border border-[#DADADA] bg-white p-1">
            <span className="inline-flex min-h-10 shrink-0 items-center gap-2 rounded-full bg-[#F0F0EF] px-3 text-sm font-semibold text-[#525252]"><Search className="h-4 w-4" />Semua</span>
            <Link href={`${getCreatorPortfolioHref(username)}?view=grid`} className={`inline-flex min-h-10 shrink-0 items-center gap-2 rounded-full px-3 text-sm font-semibold ${!isList ? `${darkButtonClass}` : "text-[#525252]"}`}><Grid2X2 className="h-4 w-4" />Grid</Link>
            <Link href={`${getCreatorPortfolioHref(username)}?view=list`} className={`inline-flex min-h-10 shrink-0 items-center gap-2 rounded-full px-3 text-sm font-semibold ${isList ? `${darkButtonClass}` : "text-[#525252]"}`}><List className="h-4 w-4" />List</Link>
          </div>
        </div>
...
function PortfolioVideoCard({ video, list }: { video: ProfileVideo; list?: boolean }) {
  ...
  return (
    <Link href={getVideoDetailHref(video.publicSlug)} className="group block min-w-0">
      <article className={`${cardClass} h-full overflow-hidden rounded-[1.5rem] border p-3 transition hover:-translate-y-0.5 hover:border-[#111111] sm:rounded-[1.75rem] ${list ? "md:flex md:gap-4" : ""}`}>
        <div className={`overflow-hidden rounded-[1.25rem] bg-[#EFEDEA] ${list ? "md:w-72 md:shrink-0" : ""}`}>
        ...
        <div className="space-y-3 p-2 sm:p-3">
          ...
          <p className="line-clamp-3 text-sm leading-6 text-[#525252]">{createTextExcerpt(video.description, 140) || "Deskripsi pendek belum ditambahkan."}</p>
          <div className="flex flex-wrap items-center justify-between gap-3 text-xs font-semibold text-[#8A8A8A]"><span className="inline-flex items-center gap-1"><CalendarDays className="h-3.5 w-3.5" />{postedLabel}</span><span className="text-[#111111]">Lihat Detail</span></div>
        </div>
      </article>
    </Link>
  );
