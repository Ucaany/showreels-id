...
const PLATFORM_BRAND_MAP: Record<
  string,
  { color: string; lightBg: string; ringColor: string }
> = {
  "Google Drive": { color: "#4285F4", lightBg: "#E8F0FE", ringColor: "rgba(66,133,244,0.35)" },
  YouTube: { color: "#FF0000", lightBg: "#FFEBEE", ringColor: "rgba(255,0,0,0.28)" },
  Instagram: { color: "#E4405F", lightBg: "#FCE4EC", ringColor: "rgba(228,64,95,0.28)" },
  Vimeo: { color: "#1AB7EA", lightBg: "#E1F5FE", ringColor: "rgba(26,183,234,0.28)" },
  Facebook: { color: "#1877F2", lightBg: "#E3F2FD", ringColor: "rgba(24,119,242,0.28)" },
};
...
// Separate interactive hero mockup component so it can hold its own cycling state
function HeroPhoneMockup({
  ...
  // Auto-cycle unless reduced motion or user has locked a row
  useEffect(() => {
  ...
  }, [prefersReducedMotion, lockedIndex, rows.length]);
  ...
  function handleRowClick(index: number) {
    ...
    lockTimerRef.current = setTimeout(() => setLockedIndex(null), 4000);
  }

  const hasAvatarImage = avatarUrl.trim().length > 0;

  return (
    <div className="aspect-[9/15.75] rounded-[2.28rem] border-[2.5px] border-slate-900/45 bg-transparent p-[2px] shadow-none ring-1 ring-slate-200/80 mx-auto w-full max-w-[318px] sm:max-w-[336px]">
      <div
        className={cn(
          "relative flex h-full flex-col overflow-hidden bg-gradient-to-b px-3 pb-3.5 pt-3 backdrop-blur-[1px] sm:px-3.5 rounded-[2.02rem] space-y-1",
          gradientClassName
        )}
      >
        {/* Background video — full brightness, light scrim only at edges */}
        {backgroundVideoSrc ? (
          <>
            <video
            ...
            {/* Subtle edge scrim — top and bottom only, centre stays clear */}
            <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(4,12,28,0.55)_0%,rgba(4,12,28,0.0)_28%,rgba(4,12,28,0.0)_68%,rgba(4,12,28,0.52)_100%)]" />
          </>
        ) : null}
        ...
        {/* Avatar */}
        <div className="relative z-10 mx-auto w-fit rounded-full border-2 border-white/55 bg-white/92 mt-3 p-1.5">
          {!hasAvatarImage ? (
        ...
        {/* Name */}
        <p className="relative z-10 mt-2.5 text-center font-bold tracking-[-0.02em] text-[1.2rem] sm:text-[1.34rem] text-white">
          {name}
        </p>

        {/* Interactive platform rows */}
        <div className="relative z-10 shrink-0 mt-3 space-y-2">
          {rows.map((row, index) => {
            const RowIcon = row.icon;
            const isActive = index === activeIndex;
            const brand = PLATFORM_BRAND_MAP[row.label];
            ...
            return (
              <button
                ...
                className={cn(
                  "flex w-full items-center justify-between rounded-2xl border text-slate-800 min-h-[2.6rem] px-3 py-2 transition-all duration-300",
                  isActive
                    ? "border-white/90 bg-white/98 shadow-md"
                    : "border-white/70 bg-white/96 hover:bg-white/98"
                )}
                style={
                  isActive && brand
                    ? { boxShadow: `0 0 0 2px ${brand.ringColor}, 0 2px 8px rgba(0,0,0,0.12)` }
                    : undefined
                }
                aria-pressed={isActive}
              >
                <span className="inline-flex min-w-0 items-center gap-2 font-semibold tracking-[-0.01em] text-[0.82rem]">
                  <span
                ...
                {/* Status badge — pulse dot when active */}
                <span
                  className={cn(
                    "flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase transition-all duration-300",
                    isActive
        ...
        {footer ? (
          <p className="relative z-10 mt-auto pt-3 text-center text-[0.69rem] font-medium tracking-[0.08em] text-white/60">
            {footer}
          </p>
        ) : null}
      </div>
