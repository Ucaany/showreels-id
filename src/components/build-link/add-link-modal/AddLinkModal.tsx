...
export function AddLinkModal({ open, onClose, onCreated, isLimitReached, maxLinksLabel, planName, portfolioHref = "", draftMode = false }: Props) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9995] isolate flex items-center justify-center bg-zinc-950/60 p-3 sm:p-5" role="dialog" aria-modal="true" aria-labelledby="add-link-modal-title">
      <button type="button" className="absolute inset-0 cursor-default" aria-label="Close add link modal backdrop" onClick={onClose} />
      <div className="relative z-[9996] mx-auto flex h-[min(85dvh,700px)] w-full max-w-[900px] flex-col overflow-hidden rounded-[1.25rem] border border-zinc-200 bg-white shadow-2xl sm:h-[min(82vh,700px)] sm:rounded-[1.5rem] lg:max-w-[940px]">
