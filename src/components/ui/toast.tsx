...
export function Toast({ 
  ...
  return (
    <div className={cn(
      "pointer-events-auto flex w-full max-w-sm items-start gap-3 rounded-2xl border p-4 shadow-lg shadow-slate-900/5 max-sm:p-3",
      style.card
    )}>
      <div className={cn(
      ...
      <div className="min-w-0 flex-1">
        ...
        {description && (
          <p className="mt-0.5 truncate text-xs leading-5 text-slate-600">{description}</p>
        )}
      </div>
      {onClose && (
        <button 
          onClick={onClose}
          className="rounded-lg p-1 text-slate-400 hover:bg-white/60 hover:text-slate-900"
