...
export default function CSFAQSection() {
  const { lang } = useLang();
  const isEN = lang === "EN";
  const t = isEN ? customerServiceEN : customerService;
  const [openIdx, setOpenIdx] = useState<number | null>(0);

  return (
    <section id="cs-faq" className="relative py-12 md:py-14">
      ...
      <div className="container mx-auto max-w-[760px] px-6">
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <h2 className="text-section-display font-semibold text-ink">
            {t.faqsHeading}{" "}
            <span className="font-accent text-accent">{t.faqsHeadingAccent}</span>
            {" "}
            {t.faqsHeadingSuffix}
          </h2>
        </div>
