...
## Astro Hybrid Integration

Project sekarang mendukung mode hybrid:

- `Next.js` tetap untuk auth, dashboard, billing, dan API.
- `Astro` dipakai untuk marketing/landing di `apps/marketing`.
...
- Next app: `http://localhost:3002` (atau port Next yang aktif)
- Astro marketing: `http://localhost:4321`
...
### Konfigurasi URL App (Astro -> Next)

Untuk tombol login/dashboard pada Astro, set env:

```bash
PUBLIC_NEXT_APP_URL=http://localhost:3002
```

Jika tidak diisi, fallback ke `http://localhost:3002`.
